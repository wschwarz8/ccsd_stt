(function(window) {
	...
	var GameGlobals = {
		...
		path: function() {
			return document.location.protocol + "//thingthingapp.com/" + Array.prototype.slice.call(arguments).join("");
		},
	...
	var ThingThing = new Class({
		initialize: function() {
			...
			if (window.ThingThingStyle && window.ThingThingStyle === "white")
		...
		isThingThingElement: function(el) {
	...
	var Menu = new Class({
		...
		generate: function(parent) {
			...
			this.container.id = 'thingthing-profile-menu';
	...
	var MenuManager = new Class({
		...
		create: function() {
			...
			this.container.id = 'thingthing-menu';
			this.container.style.bottom = '-250px';
			this.container.style.display = 'none';
			document.body.appendChild(this.container);
			this.container.innerHTML = '<div id="thingthing-pointstab" class="KICKASSELEMENT">' + '<iframe src="' + GameGlobals.path('hello.html') + '" class="KICKASSELEMENT" id="thingthing-hello-sunshine"></iframe>' + '<div id="thingthing-bomb-menu" class="KICKASSELEMENT KICKASShidden">' + '<ul id="thingthing-bomb-list" class="KICKASSELEMENT">' + '</ul>' + '</div>' + '<div id="thingthing-weapons-menu" class="KICKASSELEMENT KICKASShidden" style="display:none">' + '<ul id="thingthing-weapons-list" class="KICKASSELEMENT">' + '</ul>' + '</div>' + '<div id="thingthing-pointstab-wrapper" class="KICKASSELEMENT">' + '<div id="thingthing-points" class="KICKASSELEMENT">' +
				this.numPoints + '</div>' + '<div id="thingthing-esctoquit" class="KICKASSELEMENT">Press esc to quit</div>' +
				this.getShareHTML() + '</div>' + '<ul id="thingthing-pointstab-menu" class="KICKASSELEMENT">' + '<li class="KICKASSELEMENT"><a class="KICKASSELEMENT" id="thingthing-link-highscores" href="#">Submit score</a></li>' + '<li class="KICKASSELEMENT"><a class="KICKASSELEMENT" id="thingthing-link-menu" href="#">Menu</a></li>' + '<li class="last-li KICKASSELEMENT"><a class="KICKASSELEMENT" id="thingthing-link-ships" href="#">Switch ship</a></li>' + '</ul>' + '</div>';
			this.pointsTab = document.getElementById('thingthing-pointstab');
			this.pointsTabWrapper = document.getElementById('thingthing-pointstab-wrapper');
			this.points = document.getElementById('thingthing-points');
			this.escToQuit = document.getElementById('thingthing-esctoquit');
			this.weaponsMenu = document.getElementById('thingthing-weapons-menu');
			this.weaponsList = document.getElementById('thingthing-weapons-list');
			this.bombLink = document.getElementById('thingthing-bomb-menu');
			this.submitScoreLink = document.getElementById('thingthing-link-highscores');
			this.menuLink = document.getElementById('thingthing-link-menu');
			this.switchShipLink = document.getElementById('thingthing-link-ships');
		...
		getShareHTML: function() {
			if (this.game.isCampaign()) {
				...
				return '<iframe class="KICKASSELEMENT thingthing-like" src="//www.facebook.com/plugins/share_button.php?href=' + url + '&amp;send=false&amp;layout=button&amp;width=47&amp;show_faces=false&amp;action=like&amp;colorscheme=dark&amp;font=arial&amp;height=21" scrolling="no" frameborder="0" style="border:none; overflow:hidden; width:80px; height:21px;" allowTransparency="true"></iframe>';
			} else {
				return '<iframe class="KICKASSELEMENT thingthing-like" src="//www.facebook.com/plugins/like.php?href=http%3A%2F%2Fwww.facebook.com%2Fthingthingapp&amp;send=false&amp;layout=button_count&amp;width=47&amp;show_faces=false&amp;action=like&amp;colorscheme=dark&amp;font=arial&amp;height=21" scrolling="no" frameborder="0" style="border:none; overflow:hidden; width:80px; height:21px;" allowTransparency="true"></iframe>';
			}
		},
		onGameReady: function() {
		...
		addPoints: function(killed, pos) {
			...
			GameGlobals.thingthing.ui.addPointsBubbleAt(pos, points);
		},
		...
		addWeapon: function(weapon, id) {
			...
			li.className = 'KICKASSELEMENT thingthing-weapon-item';
			li.weapon = weapon;
			li.style.backgroundImage = 'url(' + GameGlobals.path('css/gfx/thingthing/weap-' + weapon.id + '.png') + ')';
		...
		getLowestBubbleY: function() {
			...
			for (var id in this.messages)
				if (this.messages.hasOwnProperty(id))
					top = Math.max(this.messages[id].thingthingto + this.messages[id].offsetHeight + 10, top);
			return top;
		}
	});
	...
	var alienImage = (function() {
		var image = new Image();
		image.src = GameGlobals.path("css/gfx/thingthing/alien.png");
		return image;
	});
	var alienSize = {
		w: 41,
	...
	GameGlobals.laserImage.src = GameGlobals.path('css/gfx/thingthing/laser.png');
	...
	var SheetRaphael = new Class({
		initialize: function(rect) {
			...
			GameGlobals.thingthing.registerElement(this.raphael.canvas);
		},
		...
		updateCanvas: function() {
			...
			this.raphael.canvas.style.left = GameGlobals.thingthing.scrollPos.x + (this.rect.pos.x - this.rect.size.width / 2) + 'px';
			this.raphael.canvas.style.top = GameGlobals.thingthing.scrollPos.y + (this.rect.pos.y - this.rect.size.height / 2) + 'px';
		},
		...
		destroy: function() {
			GameGlobals.thingthing.unregisterElement(this.raphael.canvas);
			this.raphael.remove();
		}
	});
	var SheetCanvas = new Class({
		initialize: function(rect) {
			...
			GameGlobals.thingthing.registerElement(this.canvas);
		...
		updateCanvas: function() {
			...
			this.canvas.style.left = GameGlobals.thingthing.scrollPos.x + (this.rect.pos.x - this.rect.size.width / 2) + 'px';
			this.canvas.style.top = GameGlobals.thingthing.scrollPos.y + (this.rect.pos.y - this.rect.size.height / 2) + 'px';
		},
		drawLine: function(xFrom, yFrom, xTo, yTo) {
			this.ctx.save();
			this.ctx.translate(this.rect.size.width / 2, this.rect.size.height / 2);
		...
		destroy: function() {
			GameGlobals.thingthing.unregisterElement(this.canvas);
			this.canvas.parentNode.removeChild(this.canvas);
		}
	});
	var Sheet = new Class({
	...
	var initIt = function() {
		// If an instance of ThingThing is already present, we add a player
		if (!window.THING) {
			window.THING = GameGlobals.thingthing = new ThingThing();
			window.THING.begin();
		} else
			window.THING.addPlayer();
	};
